...
async function loadExams() {
    try {
        ...
        if (response.ok) {
            exams = await response.json();
            console.log('Loaded exams:', exams); // Debug log
            displayExams();
        } else {
            console.error('Failed to load exams');
            exams = [];
...
function displayExams() {
    ...
    exams.forEach(exam => {
        const statusClass = `status-${exam.status}`;
        // Properly handle both _id and id fields
...
async function saveExam() {
    ...
    try {
        ...
        if (response.ok) {
            bootstrap.Modal.getInstance(document.getElementById('examModal')).hide();
            await loadExams(); // Reload exams from server
            alert(currentExamId ? 'Exam updated successfully!' : 'Exam created successfully!');
        } else {
            const errorData = await response.json();
            alert('Error: ' + (errorData.message || 'Failed to save exam'));
...
function manageQuestions(examId) {
    currentExamId = examId;
    const exam = exams.find(e => (e._id === examId || e.id === examId));
    if (!exam) {
    ...
    // Hide add question form initially
    document.getElementById('addQuestionCard').style.display = 'none';
...
async function loadQuestions(examId) {
    try {
        const token = localStorage.getItem('token');
        console.log('Loading questions for exam ID:', examId); // Debug log
...
/*function displayQuestions() {
    ...
    questions.forEach((question, index) => {
        // Handle both _id and id fields properly
        const questionId = question._id || question.id;
        console.log(`Question ${index + 1} ID:`, questionId); // Debug log
        
        if (!questionId) {
            console.warn(`Question at index ${index} has no ID:`, question);
            return;
        }
        ...
        const questionHtml = `
            <div class="question-item mb-3 p-3 border rounded">
                <div class="d-flex justify-content-between align-items-start">
                    <div class="flex-grow-1">
                        ...
                        <div class="options-list">
                            ${question.options && question.options.map((option, optIndex) => `
                                <div class="option-item mb-1 ${optIndex === question.correctAnswer ? 'correct-option text-success fw-bold' : ''}">
                                    ${String.fromCharCode(65 + optIndex)}. ${option}
                                    ${optIndex === question.correctAnswer ? '<i class="fas fa-check text-success ms-2"></i>' : ''}
                                </div>
                            `).join('')}
                    ...
                    <div class="ms-3">
                        <button class="btn btn-sm btn-outline-danger" onclick="deleteQuestion('${questionId}')" title="Delete Question">
                            <i class="fas fa-trash"></i>
                        </button>
                    </div>
                </div>
...
function displayQuestions() {
    ...
    questions.forEach((question, index) => {
        ...
        const questionText = question.text || question.question || 'No question text';
        const options = question.options || [];
        const correctAnswer = typeof question.correctAnswer === 'number' ? question.correctAnswer : null;
        const questionId = question._id || question.id;

        const optionsHtml = options.map((option, optIndex) => `
            <div class="option-item mb-1 ${optIndex === correctAnswer ? 'correct-option text-success fw-bold' : ''}">
                ${String.fromCharCode(65 + optIndex)}. ${option}
                ${optIndex === correctAnswer ? '<i class="fas fa-check text-success ms-2"></i>' : ''}
            </div>
        `).join('');
...
function addNewQuestion() {
    ...
    if (addQuestionCard) {
        ...
        console.error('Add question card element not found');
    }
}
...
async function saveQuestion() {
    ...
    try {
        const token = localStorage.getItem('token');
        console.log('Saving question for exam ID:', currentExamId); // Debug log
        ...
        if (response.ok) {
            ...
            console.log('Question saved successfully:', newQuestion); // Debug log
...
async function deleteQuestion(questionId) {
    ...
    if (confirm('Are you sure you want to delete this question?')) {
        try {
            ...
            // Try the nested route first (more RESTful)
            let response = await fetch(`/api/exams/${currentExamId}/questions/${questionId}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            ...
            // If that fails, try the direct route
